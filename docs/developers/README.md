# 👨‍💻 Developer Documentation

**Complete guides for integrating with the Demiurge Blockchain**

> *"Eyes gaze upon you, watching as a warden does his prisoners. The code serves the will, and the will serves the flame that burns eternal."*

---

## 🚀 Quick Start

1. **[Quick Start](./QUICK_START.md)** ⚡ - Build your first dApp in 5 minutes
2. **[Getting Started](./getting-started.md)** - Complete development environment setup
3. **[RPC API Reference](./rpc-api-reference.md)** - Complete RPC method documentation
4. **[Chain Operations](./chain-operations.md)** - Interact with blockchain state
5. **[Transaction Building](./transaction-building.md)** - Create and submit transactions
6. **[Module Integration](./module-integration.md)** - Integrate blockchain modules

---

## 📚 Documentation Index

### Core Integration
- **[Getting Started](./getting-started.md)** - Development environment setup
- **[RPC API Reference](./rpc-api-reference.md)** - All available RPC methods
- **[Chain Operations](./chain-operations.md)** - Querying blockchain state
- **[Transaction Building](./transaction-building.md)** - Creating transactions

### Advanced Topics
- **[Module Integration](./module-integration.md)** - Integrating with modules
- **[Session Keys](./session-keys-integration.md)** - Temporary authentication
- **[Energy System](./energy-system.md)** - Understanding energy mechanics
- **[Consensus Integration](./consensus-integration.md)** - Validator operations

### Examples
- **[Examples](./examples/)** - Code examples and tutorials

---

## 🔗 Related Documentation

- **[Architecture Documentation](../architecture/)** - Technical architecture
- **[Deployment Guide](../deployment/)** - Deploying nodes
- **[Module Specifications](../MODULE_SPECS.md)** - Module details

---

**The flame burns eternal. The code serves the will.**
